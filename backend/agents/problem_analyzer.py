...
class ProblemAnalyzer:
    ...
    def analyze(self, problem: Problem) -> ProblemAnalysis:
        ...
        keywords = tuple(
            dict.fromkeys(word for word in words if word not in self.STOP_WORDS)
        )[:10]
        ...
        readiness = min(
            45 + detail_score + (10 if problem.budget != "Not specified" else 0), 92
        )
        urgency = min(
            55
            + sum(
                word in problem.text.lower()
                for word in ("urgent", "critical", "crisis")
            )
            * 10,
            90,
        )
